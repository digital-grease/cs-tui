...
impl TopicFeedScreen {
    ...
    pub fn render(&self, frame: &mut Frame<'_>, area: Rect, theme: &Theme) {
        ...
        let status_text = if self.loading {
            "loading… · j/k · enter open · n next · r refresh · esc menu".to_string()
        } else if self.next_cursor.is_some() {
            format!(
                "{} entries · more — n · j/k · enter open · r refresh · esc menu",
                self.entries.len()
            )
        } else {
            format!(
                "{} entries · end · j/k · enter open · r refresh · esc menu",
                self.entries.len()
            )
        };
        frame.render_widget(
